...
export default function AddAndEditTaskModal() {
  ...
  const handleAddNewTaskToDb = (e: React.FormEvent<HTMLButtonElement>) => {
    ...
    const doesStatusExists = columnNames?.some((column) => column === taskData?.status);
    ...
    if (title && status && doesStatusExists) {
      if (data) {
        ...
        const getStatusColumn = columns?.find((column: { name: string }) => column.name === status);
      ...
      if (data) {
        ...
        if (status === initialTaskColumn) {
          const updatedStatusColumn = {
            ...
            tasks: columns[getStatusColumnIndex]?.tasks?.map((task: any, index: number) => {
      ...
      <ModalBody>
        ...
        <div className="py-6">
          ...
          <div className="mt-3">
            ...
            <div className="pt-2">
              <input
                id="status"
                className={`${
                  isTaskStatusEmpty || !statusExists ? "border-red-500" : "border-stone-200"
                } border w-full p-2 rounded text-sm cursor-pointer focus:outline-none`}
            ...
            {isTaskStatusEmpty ? (
              <p className="text-xs text-red-500">Task status cannot be empty</p>
            ) : !statusExists ? (
          ...
          <div className="pt-6">
            ...
            >
              <p>{isLoading ? "Loading" : `${isVariantAdd ? "Create Task" : "Save Changes"}`}</p>
            </button>
          </div>
        </div>
      </ModalBody>
